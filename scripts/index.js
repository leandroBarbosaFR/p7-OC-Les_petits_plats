...
import { deleteAccent } from "./utils/utils.js";
...
function applyFilters(){
    ...
    if (mainFilter == "" && ingredientTags.length == 0 && appareilTags.length == 0 && ustensileTags.length == 0) {
        ...
        for (const recipe of recipes) {
            ...
            // Test de correspondance avec la recherche principale si non vide et si l'utilisateur ne mets pas des accents
            if (mainFilter != "") {
              const containsIngredients = recipe.ingredients.findIndex(list => list.ingredient.toLowerCase().includes(mainFilter.toLowerCase()));

                if (deleteAccent(recipe.name).includes(deleteAccent(mainFilter))
                || deleteAccent(recipe.description).includes(deleteAccent(mainFilter))
